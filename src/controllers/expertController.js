...
exports.createExpert = async (req, res) => {
  ...
  Expert.createExpert(name, email, phone, hashedPassword, (err, result) => {
    ...
    res.json({ success: true, message: '✅ Expert registered successfully', id: result.insertId });
  });
};
...
exports.getExpertById = (req, res) => {
  ...
  Expert.getExpertById(id, (err, results) => {
    if (err) return res.status(500).json({ error: err.sqlMessage });
    if (results.length === 0) return res.status(404).json({success: false, message: 'Expert not found' });
    res.json(results[0]);
  });
};
...
exports.updateExpertStatus = (req, res) => {
  ...
  Expert.updateExpertStatus(id, status, (err, result) => {
    if (err) return res.status(500).json({ error: err.sqlMessage });
    res.json({  success: true,message: `✅ Expert status updated to ${status === 1 ? 'Enabled' : 'Disabled'}` });
  });
};
...
exports.updateStatus = (req, res) => {
  ...
  if (status === undefined)
    return res.status(400).json({success: false, error: 'Status is required (0 or 1)' });
  ...
  Expert.updateStatus(id, status, (err, result) => {
    ...
    res.json({ success: true, message: '✅ Expert status updated successfully', id, status });
  });
};